...
function Configuration(fileOrStringOrObject) {
	...
	function fileToObject(path) {
		return stringToObject(require("fs").readFileSync(path, "utf8"));
	}
	
	function stringToObject(data) {
		return JSON.parse(data.replace(/\/\/.*$/mg, "").replace(/\/\*[\s\S]*\*\//mg, ""));
	}
		...
		for (var member in right) {
			if (typeof right[member] !== 'undefined') {
				...
				if (g || s) {
					...
					if (locked && typeof left[member] === "undefined") {
